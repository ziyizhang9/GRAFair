...
for index, (beta1, is_private, sensitive_attr, model_type) in enumerate(choices):    # if idx <= 1:
    ...
    subprocess.run(f"python main_VGPF_income.py --beta1 {beta1} --is_private {is_private} --sensitive_attr {sensitive_attr} --model_type {model_type} --log_path_name {log_path_name} --output_emd_dim {output_emd_dim}" 
