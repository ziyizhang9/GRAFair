...
beta1_choices1 = [0.001,0.005,0.01,0.05,0.1,0.5,0.75]
...
for index, (beta1, is_private, dataset, lr, epochs, num_layers, model_type) in enumerate(choices):    # if idx <= 1:
    log_path_name = f"../log/GRAFair_{dataset}_{num_layers}_{model_type}_{beta1}_{lr}_{epochs}_1"
    subprocess.run(f"python main_VGPF.py --beta1 {beta1} --is_private {is_private} --model_type {model_type} --log_path_name {log_path_name} --output_emd_dim {output_emd_dim} --dataset {dataset} --lr {lr} --epochs {epochs} --num_layers {num_layers}" 
