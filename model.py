...
class VGPF(torch.nn.Module):
    ...
    def encode(self, data, record_Z=False, isplot=False):
        reg_info = {}
        x, edge_index, edge_weight = data.x, data.train_pos_edge_index, data.edge_attr
        if self.model_type in ["GCN","SAGE","Cheb"]:
    ...
    def forward(self, data, pos_edge_index, neg_edge_index, record_Z=False, isplot=False):
...
def train_VGPF(
    ...
    for epoch in range(1, epochs + 1):
        ...
        metrics = test_model_lp(model, data)
        if metrics["val_auc"] > best_metrics["val_auc"]:
